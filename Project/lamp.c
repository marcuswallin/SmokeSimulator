...
void draw_lamp_model(Model *mod, mat4 mtw, mat4 cam, GLint trans_x, GLint trans_y, GLint trans_z);
void draw_cord_model(Model *mod, mat4 mtw, mat4 cam, GLint trans_x, GLint trans_y, GLint trans_z);
void draw_lamp_with_cord(GLuint program, mat4 mtw, mat4 cam, GLint x,GLint y,GLint z);
void add_lamp(GLint x, GLint y, GLint z);
void initLamp();
...
void initLamp()
{
  ...
  //LoadTGATexture("objects/lamp_texture.tga", &lamp_tex);
  glActiveTexture(GL_TEXTURE4);
  //LoadTGATexture("objects/black.tga", &cord_tex);
...
void draw_lamp_model(Model *mod, mat4 mtw, mat4 cam, GLint x_trans, GLint y_trans, GLint z_trans)
{
...
void draw_cord_model(Model *mod, mat4 mtw, mat4 cam, GLint x_trans, GLint y_trans, GLint z_trans)
{
  mtw = Mult(T(x_trans,y_trans,z_trans),mtw);
  mat4 tot = Mult(cam, mtw);
...
void draw_lamp_with_cord(GLuint program, mat4 mtw, mat4 cam, GLint x, GLint y, GLint z)
{
  ...
  glUniform1iv(glGetUniformLocation(program, "isAlive"),
	...
	glUniformMatrix4fv(glGetUniformLocation(program_lamp, "projMatrix"), 1, GL_TRUE, projectionMatrix.m);
	mtw = S(0.7,0.7,0.7);
	glUniformMatrix4fv(glGetUniformLocation(program_lamp, "camMatrix"), 1, GL_TRUE, cam.m);
	//glUniform1i(glGetUniformLocation(program_lamp, "tex"), 3);
	draw_lamp_model(lamp_model, mtw, cam,x,y,z);

	//DRAW CORD------------------------------------------------
	glUseProgram(program_cord);
	glUniformMatrix4fv(glGetUniformLocation(program_cord, "projMatrix"), 1, GL_TRUE, projectionMatrix.m);
	mtw = S(1.5,1.5,1.5);
	glUniformMatrix4fv(glGetUniformLocation(program_cord, "camMatrix"), 1, GL_TRUE, cam.m);
	//glUniform1i(glGetUniformLocation(program_cord, "tex"), 4);
...
void clear_lamps(GLuint program)
{
  ...
  glUniform1iv(glGetUniformLocation(program, "isAlive"),
	4, isAlive);
  //glUniform1iv(glGetUniformLocation(program, "lightSourcesDirPosArr"),
	//4, lightSourcesDirectionsPositions);
